...
class DataGenerator:
    ...
    def parse_pcb(self, labels_filename, img_channels=1, ret=False):
        ...
        with open(self.labels_filename) as labels_file:
            for img_anno_pair in labels_file:
                ...
                if img_channels == 2:
                    self.images.append(np.stack((test_img, temp_img), axis=-1))
                if img_channels == 3:
                    self.images.append(np.stack((test_img, np.abs(test_img - temp_img), temp_img), axis=-1))
                box = np.array(box, dtype=np.int32)
                if np.ndim(box) == 1:
                    box = np.array([box])
                self.labels.append(box)
                self.image_ids.append(img_path[-12:-4])
            labels_file.close()
    ...
    def generate_pcb(self, batch_size=32, shuffle=True, argumentation='train', label_encoder=None, random_crop=False, resize=False, returns={'processed_images', 'encoded_labels'}, keep_images_without_gt=False, degenerate_box_handling='remove'):
        ...
        if resize and ((type(resize) is list) or (type(resize) is tuple)):
            img_height, img_width = resize[0], resize[1]
            resizemethod = 'cv2.INTER_LINEAR'

            if len(resize) == 3:
                resizemethod = resize[2]
            resize = Resize(height=img_height, width=img_width, interpolation_mode=eval(resizemethod))
        ...
        while True:
            ...
            batch_X_test, batch_X_temp, batch_y = [], [], []
            ...
            if not (self.images is None):
                for i in batch_indices:
                    if self.img_channels == 1:
                        batch_X_test.append(self.images[i][0].copy())
                        batch_X_temp.append(self.images[i][1].copy())
                    if self.img_channels in [2, 3]:
                        batch_X_test.append(self.images[i].copy())
                if not (self.filenames is None):
                    batch_filenames = self.filenames[current:current + batch_size]
                else:
                    batch_filenames = None
            else:
                raise ValueError('The images is None!')
            batch_y = deepcopy(self.labels[current:current + batch_size])
            batch_image_ids = self.image_ids[current:current + batch_size]

            if 'original_images' in returns:
                if self.img_channels == 1:
                    batch_original_images = deepcopy([batch_X_test, batch_X_temp])  # The original, unaltered images
                if self.img_channels in [2, 3]:
                    batch_original_images = deepcopy(batch_X_test)  # The original, unaltered images
            ...
            for i in range(len(batch_X_test)):
                ...
                test_img = batch_X_test[i]
                if self.img_channels == 1:
                    temp_img = batch_X_temp[i]
                    assert test_img.shape == temp_img.shape
                bbox = batch_y[i]

                # Apply any image transformations we may have received.
                if argumentation in ['train', 'Train', 'TRAIN']:
                    ...
                    if True:
                        ...
                        transaxis = (1, 0) if self.img_channels == 1 else (1, 0, 2)
                        if angle == 270:
                            test_img = np.transpose(test_img, transaxis)
                            test_img = cv2.flip(test_img, 0)
                            if self.img_channels == 1:
                                temp_img = np.transpose(temp_img, transaxis)
                                temp_img = cv2.flip(temp_img, 0)
                        elif angle == 180:
                            test_img = cv2.flip(test_img, -1)
                            if self.img_channels == 1:
                    ...
                    if resize:
                        if ('inverse_transform' in returns) and ('return_inverter' in inspect.signature(resize).parameters):
                            test_img, bbox, inverse_transform = resize(test_img, bbox, return_inverter=True)
                            inverse_transforms.append(inverse_transform)
                        else:
                            test_img, bbox = resize(test_img, bbox, return_inverter=False)
                        if self.img_channels == 1:
                            temp_img = resize(temp_img, None, return_inverter=False)

                if argumentation in ['test', 'Test', 'TEST', 'valid', 'Valid', 'VALID']:
                ...
                if test_img is None: # In case the transform failed to produce an output image, which is possible for some random transforms.
                ...
                if not (self.labels is None):
                    ...
                    if np.any(bbox[:,xmax] - bbox[:,xmin] <= 0) or np.any(bbox[:,ymax] - bbox[:,ymin] <= 0):
                        if degenerate_box_handling == 'warn':
                            warnings.warn("Detected degenerate ground truth bounding boxes for batch item {} with bounding boxes {}, ".format(i, bbox) +
                            ...
                            if (bbox.size == 0) and not keep_images_without_gt:
                                batch_items_to_remove.append(i)


                batch_X_test[i] = test_img
                if self.img_channels == 1:
                    batch_X_temp[i] = temp_img
                batch_y[i] = bbox
            ...
            if self.img_channels == 1:
                batch_X_temp = np.array(batch_X_temp)
                batch_X_test = np.expand_dims(batch_X_test, 3)
                batch_X_temp = np.expand_dims(batch_X_temp, 3)

            if (batch_X_test.size == 0):
            ...
            if self.img_channels == 1:
                batch_X = [batch_X_test, batch_X_temp]
            else:
                batch_X = batch_X_test
